...
export class CodeInputHandler {
  ...
  async enterVerificationCode(verificationCode) {
    console.log('📝 [CodeInputHandler] Entering verification code...');
    ...
    console.log('✅ [CodeInputHandler] Verification code entered successfully!');
  ...
  async trySelector(selector) {
    try {
      ...
      for (const candidate of candidates) {
        ...
        if (isValidField) {
          console.log(`✅ [CodeInputHandler] Found valid code field with selector: ${selector}`);
          return candidate;
        }
      }
    } catch (e) {
      console.log(`⚠️ [CodeInputHandler] Error with selector "${selector}": ${e.message}`);
    }
    return null;
  }
